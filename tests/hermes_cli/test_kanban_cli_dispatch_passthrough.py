...
def test_cli_dispatch_passes_max_in_progress_from_config(isolated_kanban_home, monkeypatch):
    ...
    assert captured.get("stale_timeout_seconds") == 600


def test_cli_dispatch_disables_stale_recovery_for_invalid_config(
    isolated_kanban_home, monkeypatch
):
    """An invalid stale timeout must fail closed without breaking dispatch."""
    from hermes_cli import kanban as kb_cli
    from hermes_cli import kanban_db

    monkeypatch.setattr(
        "hermes_cli.config.load_config",
        lambda: {"kanban": {"dispatch_stale_timeout_seconds": "invalid"}},
    )
    captured = {}
    monkeypatch.setattr(
    ...
    assert captured.get("stale_timeout_seconds") == 0
