...
#   * HERMES_TEST_CONTROLLER_RECEIPT_CLI selects a compiled local controller
#     CLI for the opt-in isolated signed-receipt integration test.
#
...
for _test_var in HERMES_TEST_IMAGE HERMES_TEST_WORKERS HERMES_TEST_PATHS \
  ...
  HERMES_TEST_CONTROLLER_RECEIPT_CLI HERMES_TEST_CONTROLLER_SETUP_URL; do
  if [ -n "${!_test_var:-}" ]; then
